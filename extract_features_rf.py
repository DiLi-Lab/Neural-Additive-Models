...
def get_avg_word_embedding_spacy(input_word_list, lemma=True, skip_list=['nan']):
    ...
    if counter > 0:
        out_vector /= counter
    if lemma:
        return out_vector, ['lexical_word_embedding_spacy_lemma_' + str(a) for a in
                            range(len(out_vector))]  # noqa: E501
    else:
...
def get_features_df_list_lexical(df_list):
    data_number = 0
    feature_names = []
    for cur_df in tqdm(df_list, desc='Extracting lexical features: '):
        cur_word_list = np.array(
            list(cur_df['word']), dtype=str,
        ).tolist()
...
def get_features_from_df_list_numeric(
    ...
    data_number = 0
    for cur_df in tqdm(df_list, desc='Extracting numerical features: '):
        feature_id = 0
...
def get_features_from_df_list_categorical(
    ...
    out_feature_matrix = np.zeros([len(df_list), feature_number])
    data_number = 0
    for cur_df in tqdm(df_list, desc='Extracting categorical features: '):
        feature_id = 0
        for use_col, cur_feature_values in zip(use_cols, feature_values):
            # print(use_col)
...
def get_combined_features(df_list):
    ...
    data_arr = np.hstack(
        [data_arr_numeric, lexical_features],
    )
    # feature_names = list(feature_names_numeric) + list(feature_names_cat) + \
    #                 list(lexical_feature_names)

    feature_names = list(feature_names_numeric) + list(lexical_feature_names)
