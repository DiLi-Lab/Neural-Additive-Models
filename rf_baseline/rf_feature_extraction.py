...
def get_linguistic_features_for_lists(
    ...
    deps, n_rights, rights, n_lefts, lefts, dep_distance = parse_dependency(' '.join(word_list))
    ...
    for w_i in range(len(word_list)):
        ...
        cluster_id = get_word_cluster(cur_word,word_cluster_thresholds = word_cluster_thresholds)
        if cluster_id != -1:
    ...
    counter = 0
    unique_content_word_list = list(np.unique(list(content_word_dict.values())))
    ...
    for content_word in unique_content_word_list:
        ...
        content_word_id_dict[content_word] = num_numeric_features + num_entity_feature + num_pos_feature + counter
        counter += 1
    ...
    for r_pos in unique_reduced_pos_list:
        ...
        reduced_pos_id_dict[r_pos] = num_numeric_features + num_entity_feature + num_pos_feature + num_content_word_feature + counter
        counter += 1
        
    feature_matrix = np.zeros([num_numeric_features + num_entity_feature +\
    ...
    for w_i in range(len(word_list)):
        ...
        cur_fixations = fixation_list[w_i]
        cur_sentence = sentence_id_list[w_i]
        cur_regression_values = np.array(regression_values[w_i],dtype=np.float32)
        ...
        if len(cur_fixations) > 0:            
            ...
            # WFC_FF_nomarlized
            wfc_ff_norm = ff / np.mean(complete_sentence_ff_fixations[cur_sentence])
            feature_matrix[2,w_i] = wfc_ff_norm
            # WFC_TF_nomarlized
            wfc_tf_norm = tf / np.mean(complete_sentence_tf_fixations[cur_sentence])
            ...
            if cluster_id != -1:
                # IC_FF__normalized
                ic_ff_normalized = ff / np.mean(word_cluster_threshold_ff_fixations[cluster_id])
                feature_matrix[6,w_i] = ic_ff_normalized
                # IC_TF__normalized
                ic_tf_normalized = tf / np.mean(word_cluster_threshold_tf_fixations[cluster_id])
...
def get_rf_features_for_word_features(data,feature_names,
                                    disable = True,
    ...
    for i in tqdm(np.arange(data.shape[0]),disable = disable):
        ...
        word_len_feature_id = np.where(np.array(feature_names) == 'word_len')[0]
        word_lens = np.array([float(cur_instance[a,word_len_feature_id]) for a in range(cur_instance.shape[0])])
        ...
        fixation_indicator = np.array([int(cur_instance[a,fixation_feature_id] > 0) for a in range(cur_instance.shape[0])])
        ...
        get_not_is_feature_names = np.array(feature_names)[get_not_is_feature_ids]
        ...
        for j in range(len(get_is_feature_names)):
            ...
            cur_fixation_on_is_feature_ids = np.logical_and(fixation_indicator > 0,
            ...
            for k in range(len(get_not_is_feature_names)):
                ...
                cur_vals = cur_instance[cur_fixation_on_is_feature_ids,cur_not_is_feature_id]
                if i == 0:
                    new_feature_names.append(cur_is_feature + '_' + cur_not_is_feature_name)
                if len(cur_vals) > 0:
        ...
        if use_gaze_entropy_features:
            ...
            gaze_features, gaze_feature_names = get_gaze_entropy_features(x_means,
                                  y_means)
...
def get_gaze_entropy_features(x_means,
                              y_means,
                             x_dim = 1024,
                             y_dim = 768,
                             patch_size = 64):
...
def get_lexical_features(input_word_list):
    features_entity, feature_names_entity = get_entity_count_features(input_word_list)
    features_pos, feature_names_pos = get_pos_count_features(input_word_list)
    features_we, feature_names_we = get_avg_word_embedding_spacy(input_word_list,lemma=False)
    features_we_lemma, feature_names_we_lemma = get_avg_word_embedding_spacy(input_word_list,lemma=True)
    
    
    lexical_features = np.hstack([features_entity,features_pos,features_we, features_we_lemma])
...
def get_lexical_features_df_list(df_list):
    ...
    for cur_df in tqdm(df_list):
        cur_word_list = np.array(list(cur_df['CURRENT_FIX_INTEREST_AREA_LABEL']), dtype=np.str)
        lexical_features, lexical_feature_names = get_lexical_features(cur_word_list)
        if data_number == 0:
    ...
    for cur_df in tqdm(df_list):
        ...
        for use_col in use_cols:
            ...
            for aggregation_function in feature_aggregations:
                ...
                cur_feature_name = str(use_col) + '_' + str(aggregation_function)
                if data_number == 0:
    ...
    for cur_df in tqdm(df_list):
        ...
        for use_col, cur_feature_values in zip(use_cols,feature_values):
            ...
            for feature_value in cur_feature_values:
                ...
                if data_number == 0:
                    cur_feature_name = str(use_col) + '_' + str(feature_value) + '_count'
                    feature_names.append(cur_feature_name)
                    cur_feature_name = str(use_col) + '_' + str(feature_value) + '_frac'
                    feature_names.append(cur_feature_name)
...
def get_combined_features(df_list):
    data_arr_rf_numeric, feature_names_numeric = get_features_from_df_list_numeric(df_list,
        use_cols = ['CURRENT_FIX_X',
    ...
    data_arr_rf_cat, feature_names_cat = get_features_from_df_list_categorical(df_list,
        use_cols = ['PREVIOUS_SAC_DIRECTION'],
        feature_values=[['DOWN', 'LEFT', 'RIGHT', 'UP']])
    
    lexical_features, lexical_feature_names = get_lexical_features_df_list(df_list)
    
    data_arr_rf = np.hstack([data_arr_rf_numeric,data_arr_rf_cat,lexical_features])
